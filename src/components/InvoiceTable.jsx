...
function InvoiceTable() {
  ...
  const [sub, setsub] = useState(0.00);
  ...
  const handleInputChange = (index, field, value) => {
    ...
    if (field === 'qty' || field === 'rate' || field === 'sgst' || field === 'cgst' || field === 'cess') {
  ...
  return (
    <div className="mt-4 text-[14px]">
          ...
          {tableData.map((row, index) => (
            ...
            >
              <td>
                ...
                />
                <div className=''><input type="text" placeholder='HSN/SAC' className='rounded-[2px]  text-inherit focus:ring-1 focus:ring-[#5B9AFF] hover:ring-1 placeholder:text-[#676767]' /></div>
              ...
              <td>
                ...
                />
                <div className='text-[14px] text-white mt-3'>{parseFloat(row.cess).toFixed(2)}</div>
              ...
              <td>
                <input
                  ...
                  className="w-[50.75px] h-[23.06px]  rounded-[3px]  text-inherit focus:ring-1 focus:ring-[#5B9AFF] hover:ring-1 placeholder:text-[#676767]  text-black text-center"
                />
                <div className='text-[14px] text-[#00000096] mt-3'>{parseFloat(row.sgst).toFixed(2)}</div>
              </td>
              <td>
                <input
                  type="number"
                ...
                />
                <div className='text-[14px] text-[#00000096] mt-3'>{parseFloat(row.cgst).toFixed(2)}</div>
              ...
              <td>
                ...
                />
                <p className='text-[14px] text-[#00000096] mt-3'>{parseFloat(row.cess).toFixed(2)}</p>
      ...
      <div className="flex items-center mt-4 cursor-pointer ml-2" onClick={addRow}>
        <img width="16" height="16" src="https://img.icons8.com/tiny-color/16/add.png" alt="add" />
  ...
  <div className="flex flex-col justify-center items-end w-1/2 p-4">
    ...
    {row.sgst > 0 && (
    <div className="mb-4">
      <p className="text-sm font-normal">({(row.sgst / 100).toFixed(2)}%)</p>
    </div>
    )}

  {row.cgst > 0 && (
    <div className="mb-4">
      <p className="text-sm font-normal"> ({(row.cgst / 100).toFixed(2)}%)</p>
    </div>
     )}
